...
def send_outcome(Scorelist,Progress_outcome):
    for x in range(len(Scorelist)):
          text.append((Progress_outcome+str(Scorelist[x])))
...
while True:
    ...
    send_outcome(progress_list,'Progress - ')
    send_outcome(trailer_list,'Progress (module trailer) - ')
    send_outcome(retriever_list,'Module retriever - ')
    send_outcome(exclude_list,'Exclude - ')
            
    file = open('COURSE.txt','w')      
#Creation of a text file call COURSE        #cmdlinetips
    for outcome in text:                #Inserting the outcomes line by line to the file we created  
       course = file.write(outcome)
       course = file.write('\n')
    file.flush
    file.close()
            
    file = open('COURSE.txt','r')       #Getting the data in the file to the program
    course = file.read()                 
    print(course)                       #Printing the data in the file as a string in the program as exactly as saved in the file
    file.close()        
    break
